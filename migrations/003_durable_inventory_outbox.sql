...
-- (or cancelled), and workers drain it under leases. Forward-only; every
-- statement is idempotent so re-running on a partially migrated database is
-- safe. See docs/payment-inventory-reconciliation.md.
...
create table if not exists inventory_outbox (
  ...
  -- 'cancelled' is only ever set on a release row superseded by a verified payment.
  state text not null default 'pending' check (state in ('pending','processing','completed','manual_review','cancelled')),
