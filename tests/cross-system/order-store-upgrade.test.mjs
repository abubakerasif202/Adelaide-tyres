...
test("the upgraded store drives the backfilled work to completion exactly once", async () => {
  ...
  try {
    // The legacy paid-without-reservation order is now manual_review, so it is
    // the only thing notifiable before the backfilled commit runs.
    const early = await store.claimNotification("early");
    assert.equal(early.reference, "UPG-PAID-NORES");
    assert.equal(early.inventoryStatus, "manual_review");
    assert.equal(await store.claimNotification("early-2"), null, "nothing else is notifiable before the sale is committed");
    await store.finishNotification(early.checkoutSessionId, "early", true);
    const seen = [];
    ...
    // Both the newly committed order and the legacy paid-without-reservation
    // order (now manual_review) become visible to the business.
    const notified = [];
    ...
    assert.ok((await order("UPG-PAID-NORES")).notified_at);
